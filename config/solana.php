<?php
// Solana Campus Configuration
define('CAMPUS_ID', 9);
define('CAMPUS_NAME', 'Solana Campus');
define('CAMPUS_CODE', 'solana');
define('CAMPUS_DOMAIN', 'solana.csu.edu.ph');
define('CAMPUS_FULL_NAME', 'Cagayan State University - Solana Campus');
define('CAMPUS_ADDRESS', 'Solana, Cagayan Valley, Philippines');
define('CAMPUS_PHONE', '+63 XXX XXX XXXX');
define('CAMPUS_EMAIL', 'info@solana.csu.edu.ph');
define('CAMPUS_UPLOAD_PATH', __DIR__ . '/../uploads/');
define('CAMPUS_CACHE_PATH', __DIR__ . '/../cache/');
define('CAMPUS_LOG_PATH', __DIR__ . '/../logs/');
define('CAMPUS_PRIMARY_COLOR', '#7c2d12');
define('CAMPUS_SECONDARY_COLOR', '#f59e0b');
define('CAMPUS_LOGO_PATH', '/assets/img/campuses/solana-logo.png');
define('CAMPUS_FAVICON_PATH', '/assets/img/campuses/solana-favicon.ico');
define('CAMPUS_ENABLE_BLOG', true);
define('CAMPUS_ENABLE_EVENTS', true);
define('CAMPUS_ENABLE_GALLERY', true);
define('CAMPUS_ENABLE_ANNOUNCEMENTS', true);
define('CAMPUS_ENABLE_CONTACT_FORM', true);
define('CAMPUS_TIMEZONE', 'Asia/Manila');
define('CAMPUS_LANGUAGE', 'en');
define('CAMPUS_LOCALE', 'en_PH');
require_once __DIR__ . '/database.php';
require_once __DIR__ . '/constants.php';
?>
